...
def getSubscriber(subscriptionId):
    subscriber = db.session.execute(db.select(Subscription).filter_by(subscription_id=subscriptionId)).scalar_one()
    print(subscriber)
...
def updateSubscriber(subscriptionId, notificationFreq, notifyOnPriceChange, sendNotification):
