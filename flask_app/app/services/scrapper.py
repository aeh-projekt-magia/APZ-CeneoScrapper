...
#                 single_review = {
#                     key: self.extract_element(review, *values)
#                     for key, values in self.review_elements.items()
#                 }
#                 single_review["review_id"] = review["data-entry-id"]

#                 single_review["recommendation"] = (
#                     True
#                     if single_review["recommendation"] == "Polecam"
#                     else False
#                     if single_review["recommendation"] == "Nie polecam"
#                     else None
#                 )
#                 single_review["stars"] = float(
#                     single_review["stars"].split("/").pop(0).replace(",", ".")
#                 )
#                 single_review["useful"] = int(single_review["useful"])
#                 single_review["useless"] = int(single_review["useless"])
#                 single_review["publish_date"] = (
#                     single_review["publish_date"].split(" ").pop(0)
#                     if single_review["publish_date"] is not None
#                     else None
#                 )
#                 single_review["purchase_date"] = (
#                     single_review["purchase_date"].split(" ").pop(0)
#                     if single_review["purchase_date"] is not None
#                     else None
#                 )
