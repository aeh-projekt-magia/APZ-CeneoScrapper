...
class ProductSubscription:
    ...
    def __get_objects__(user_id, product_id):
        ...
        product_to_subscribe = db.session.query(Products).where(Products.id == product_id).first()
        return user, product_to_subscribe

    @staticmethod
    def add(user_id, product_id):
        user, product_to_subscribe = ProductSubscription.__get_objects__(user_id, product_id)
