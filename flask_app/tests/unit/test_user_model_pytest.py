...
test_user = {"email": "test@test.com", "password": "strongpassword", "is_admin": False}
...
def test_user_model_add(app):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
        user = db.session.execute(
            db.select(User).filter_by(email=test_user["email"])
        ).scalar_one()
...
def test_user_model_get(app, capfd):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
...
def test_user_model_get_all(app, capfd):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
...
def test_user_model_update(app):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
        updateUser(test_user["email"], "newmail@mail.com", "newpass", True)

        user = db.session.execute(
            db.select(User).filter_by(email="newmail@mail.com")
        ).scalar_one()
...
def test_user_model_delete_one(app):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
...
def test_user_model_delete_all(app, capfd):
    with app.app_context():
        addUser(test_user["email"], test_user["password"], test_user["is_admin"])
        addUser("1" + test_user["email"], "1" + test_user["password"], True)
        deleteAllUsers()
