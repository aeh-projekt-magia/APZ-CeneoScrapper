...
def test_register(client, test_user_data):
    ...
    response = client.post(
        ...
        data=dict(
            ...
            confirm=test_user_data["password"],
        ),
...
def test_register_incorrect_email(client):
    ...
    client.post(
        "/register",
        data=dict(email="test@user", password="test_user", confirm="test_user"),
        follow_redirects=True,
    )
...
def test_register_passwords_dont_match(client):
    ...
    response = client.post(
        "/register",
        data=dict(email="test@user.user", password="test_user", confirm="012345678"),
        follow_redirects=True,
    )
    user = User.query.filter_by(email="test@user.com").first()
...
def test_register_email_duplicate(client, test_user_data):
